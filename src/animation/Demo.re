...
module Counter1 = {
  ...
  let make = (~state, ~update) => {
    <div>
      <button onClick={_ => update({...state, count1: state.count1 + 1})}>
        {React.string("+")}
      </button>
      <button onClick={_ => update({...state, count1: state.count1 - 1})}>
        {React.string("-")}
...
module Counter2 = {
  ...
  let make = (~state, ~update) => {
    <div>
      <button onClick={_ => update({...state, count2: state.count2 + 1})}>
        {React.string("+")}
      </button>
      <button onClick={_ => update({...state, count2: state.count2 - 1})}>
        {React.string("-")}
...
module Toggle = {
  ...
  let make = (~state, ~update) => {
    <div>
      <button onClick={_ => update({...state, toggle: !state.toggle})}>
        {React.string("Toggle")}
      </button>
...
module GlobalStateExample = {
  type action =
    | Update(GlobalState.state);
  [@react.component]
  let make = () => {
    let (state: GlobalState.state, dispatch) =
      React.useReducer(
        (_, action) =>
          switch (action) {
          | Update(state) => state
    ...
    let update: GlobalState.state => unit = state => dispatch(Update(state));
    <div>
  ...
  let make = () => {
    let (state, dispatch) =
      React.useReducer(
        ...
        [@reason.preserve_braces] 0,
      );

    <div>
      <button onClick={_ => dispatch(Incr)}> {React.string("+")} </button>
      <button onClick={_ => dispatch(Decr)}> {React.string("-")} </button>
...
module TextInput = {
  ...
  type action =
    | Text(string);
  let textOfEvent = e => e |> ReactEvent.Form.target(e)##value;
  ...
  let make = (~renderValue) => {
    let (state, dispatch) =
      React.useReducer(
        ...
        {animation: SpringAnimation.create(0.0), value: 0.0, target: 1.0},
      );
    React.useEffect0(() => {
      state.animation
...
module TextInputRemote = {
  ...
  let textOfEvent = e => e |> ReactEvent.Form.target(e)##value;
  [@react.component]
  let make = (~remoteAction, ~onChange=_ => (), ~showText=x => x, ~initial="") => {
    let (state, dispatch) =
      React.useReducer(
        (_, action) =>
          switch (action) {
...
module Child = {
  [@react.component]
  let make = (~remoteAction) => {
    <div> {React.string("in child")} <GrandChild remoteAction /> </div>;
  };
};
